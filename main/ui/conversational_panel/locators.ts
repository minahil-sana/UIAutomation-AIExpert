...
export const conversationalPanelLocators = {
	getConversationalPanelLocators: (page: Page) => {
	...
	enablementTileByName: (page: Page, tileName: string): Locator => {
		const aiExpertPanelRoot = page.locator('[id="single-spa-application:@xcloud-workspace/aiExpertPanel"]');
		return aiExpertPanelRoot.getByRole('button', { name: tileName, exact: true });
	},
	promptQuestionByCategoryAndNumber: (page: Page, category: string, questionNumber: number): Locator => {
		const aiExpertPanelRoot = page.locator('[id="single-spa-application:@xcloud-workspace/aiExpertPanel"]');
		return aiExpertPanelRoot.getByTestId(`prompt-item-${category}-${questionNumber}`);
	},
};
